...
role, as well as the screen parameters. Other variables with obvious
meaning include \texttt{xy}, \texttt{xstar}, \texttt{xstardot}, \texttt{q1},
\texttt{q2}, q, etc.
...
The variables \texttt{q1iws}, \texttt{q2iws} are the initial angles
computed in the workspace (ws = workspace), and are used in \texttt{OFFLINE}
modes only.
...
robot control logic seen in the introduction, with \texttt{K} = 10$\times$\texttt{I}.
Internally, it has three areas: the Inversion of J area, the K$\times$Deltax
computation area and the Final computation area.
...
\texttt{sim\_struct = prepare\_simulation('ONLINE', 'PLOT\_NO\_LINKS',
1, 1, 0.8, 0.00001);}
